...
Many common sampling models belong to the **exponential family** of distributions, iincluding the binomila and Poisson distribution we saw in this chapter. A one-parameter exponential family has probability density (or mass) function of the form $$
\begin{aligned}
p(y \mid \theta) 
...
$$
With this result, and suppose the data consist of $n$ independent observations $y_1,\ldots,y_n$ are sampled from $Y_1,\dots,Y_n \stackrel{iid}{\sim} p(y\mid \theta)$, the posterior distribution is



## Exponential families and conjugate priors

The binomial and Poisson models discussed in this chapter are both instances of
**one-parameter exponential family models**.

A one-parameter exponential family model is any model whose density can be written as
$$
p(y \mid \phi) = h(y)\, c(\phi)\, \exp\{\phi\, t(y)\},
$$
where

- $\phi$ is the unknown parameter,
- $t(y)$ is a sufficient statistic,
- $h(y)$ does not depend on $\phi$,
- $c(\phi)$ is a normalizing function.

Diaconis and Ylvisaker (1979) studied conjugate prior distributions for general
exponential family models. In particular, they considered priors of the form
$$
p(\phi \mid n_0, t_0)
