...
4. why these ideas matter in modern statistics and machine learning.
...
Variational inference can be much faster than MCMC, especially in large or complicated models.

This is one reason it is widely used in machine learning and large-scale Bayesian modeling.
...
Whereas variational inference is mainly about computation, Gaussian processes are mainly about modelling.
...
A Gaussian process provides a Bayesian way to model an unknown function flexibly.

## Core idea

In Gaussian process regression, we write
...
A **Gaussian process prior** says that for any collection of inputs
...
A Gaussian process is an infinite-dimensional generalization of the multivariate Gaussian distribution.

:::

This is exactly why Gaussian processes fit naturally after our earlier lecture on multivariate Gaussian models.
...
::: {.callout-note}
Bayesian statistics does not replace machine learning.

Rather, it provides a principled probabilistic framework for many machine learning goals.
:::
...
to ideas that connect directly to modern machine learning and modern Bayesian data analysis.
