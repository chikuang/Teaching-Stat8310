...
::: {.callout-example title="Motivating example"}
...
This kind of example is one of the main motivating examples in Chapter 7 in @hoff:2009:book.
...
$$
\boldsymbol{\mu}
= \left(\mu_1,\mu_2,\dots,\mu_p\right).
$$
...
::: {.callout-definition title="Correlation matrix"}
...
::: {.callout-example title="An example of conditional distributions in a bivariate Gaussian"}
...
Because conditional Gaussian distributions are again Gaussian, missing entries in a multivariate observation can be imputed from their conditional distribution given the observed entries. This is a major application in Section 7.5 in @hoff:2009:book.
...
In @hoff:2009:book, it explains the motivation of the Wishart distribution by thinking about empirical covariance matrices. If $\mathbf{z}_1,\dots,\mathbf{z}_n$ are mean-zero multivariate vectors, then
...
```{r gibbs-mvn-simulated-data, warning=FALSE, message=FALSE}
...
@hoff:2009:book emphasizes that this is much better than either:
