...
> Leading objectives:
>
> be familar with the following concepts
>
>- Belief Functions
>- Probability
>- Bayes' Rule
...
* $\be(F)>\be(G)$ means you prefer a bet $F$ is true over that $G$ is true.

Also, we want $\be(\cdot)$ to describe our beliefs under certain conditions 

* $\be(F\mid H) > \be(G\mid H)$ means that if we knew that $H$ were true, then we would perfer to bet that $F$ is also true over $G$ is also true.

* $\be(F\mid G) > \be(F\mid H)$ means that if we bet on $F$, we would perfer to do it under the condition that $G$ is true rather than $H$ is true.
...
* **B1**: $\be(\neg H\mid H) \le \be (F \mid H) \le \be(H \mid H)$

* **B2**: $\be(F \vee G\mid H) \ge  \max\{\be(F \mid H), \be(G\mid H)\}$

* **B3**: $\be(F \wedge G\mid H)$ can be derived from $\be(G\mid H)$ and $\be(F\mid G \wedge H)$
::: 


How should we interpret these properties, and do they make sense? 

* B1 means that the number we assign to $\be(F \mid H)$, our conditional belief in $F$
given $H$, is bounded below and above by the numbers we assign to complete disbelief $\be(\neg H \mid H)$ and complete belief $\be(H \mid H)$.
...
* B3 is a bit trickier. To see why it makes sense, imagine you have to decide whether or not $F$ and $G$ are true, knowing that $H$ is true. You could do this by first deciding whether or not $G$ is true given $H$, and if so, then deciding
...
+ **P1**: 
$$
0=\pr(\neg H \mid H) \leq \pr(F \mid H) \leq \pr(H \mid H)=1
$$

+ **P2**: $$\pr(F \cup G \mid H)=\pr(F \mid H)+\pr(G \mid H),\quad \text{if}\quad F \cap G=\emptyset$$

+ **P3**: $$\pr(F \cap G \mid H)=\pr(G \mid H) \pr(F \mid G \cap H)$$
...
::: {.callout-definition title = "Partition"}
...
In the context of indetifying which of several statements is true, if $\HH$ is the set of all possible truths and $\{H_1,\dots,H_K\}$ is a partition of $\HH$, then exactly one set $H_j$ contains the truth.

::: {.callout-example title = "Model correctness"}

Let $\HH$ be the status of a statistical model. 
...
Suppose $\{H_1,\dots,H_K\}$ is a partition of $\HH$,$\pr(\HH)=1$  and $E$ is some specific event. Then, by the axioms of probability, we have

* Law of total probability
$$
\sum_{k=1}^K \pr(H_k)=\pr\left(\bigcup_{k=1}^K H_k\right)=\pr(\HH)=1
$$
...
* Bayes' rule
$$
\pr(H_j \mid E)=\frac{\pr(E \mid H_j) \pr(H_j)}{\pr(E)}=\frac{\pr(E \mid H_j) \pr(H_j)}{\sum_{k=1}^K \pr(E \mid H_k) \pr(H_k)}
$$
...
$$\{\pr(E\mid H_1), \pr(E\mid H_2),\pr(E\mid H_3), \pr(E\mid H_4)\} = \{.11, .19, .31, .53\}.$$
These probabilities do not sum to 1 - they represent the proportions of people
with college degrees in the four different income subpopulations $H_1, H_2, H_3$
and $H_4$. Now let’s consider the income distribution of the college-educated
population. Using Bayes’ rule we can obtain

$\{\pr(H_1\mid E), \pr(H_2\mid E), \pr(H_3\mid E), \pr(H_4 \mid E)\} = \{0.09, 0.17, 0.27, 0.47\} ,$
and we see that the income distribution for people in the college-educated
population differs markedly from $\{0.25, 0.25,0.25,0.25\}$, the distribution for the
...
In Bayesian inference, ${H_1, . . . ,H_K}$ often refer to disjoint hypotheses or
...
*beliefs should be* after seeing the data, it only tells us how they *should change after seeing the data*.


## Independence

::: {.callout-definition title="(Conditional) Independence"}
Two events $F$ and $G$ are conditionally independent, if given $H$, we have $\pr(F \cap G \mid H) = \pr(F\mid H)\pr(G\mid H)$.
:::
...
Thus, conditional independence implies that $\pr(F \mid H \cap G) = \pr(F\mid H)$. In other words, if we know $H$ is true, and $F$ and $G$ are conditionally independent given $H$, then knowing $G$ does not change our belief about $F$.


::: {.callout-example title = "Conditional Dependence"}
...
Think: In both of these situations, H being true implies a relationship between $F$
...
quantity about which we make probability statements.* For example, the
...
The event that the outcome $Y$ of our survey has the value $Y$ is expressed as $\{Y=y\}$. For each $y\in\mathcal{Y}$, the shorthand notation fofr $\pr(Y=y)$ is $p(y)$, and $p(\cdot)$ is called the **probability mass function** of $Y$, and with two properties
...
Let $Y$ be the number of successes in $n$ independent Bernoulli trials, each with probability of success $p$. Then, $Y$ follows a Binomial distribution with parameters $n$ and $p$, denoted as $Y \sim \mathrm{Binomial}(n,p)$. The probability mass function of $Y$ is given by $$
...
$$
p(2) = \pr(Y=2 \mid \theta=0.3) = \binom{3}{2
...
The **cummulative distribution function** (cdf) of a continuous random variable $Y$
is defined as
$$
...
Probability of various events can be derived from the cdf.
For example, for any interval $A = (a,b] \subseteq \mathcal{Y}$, we have
$$
\pr(Y \in A) = \pr(a < Y \leq b) =
F(b) - F(a).
$$
...
::: {.callout-note title = "Difference bewtwen mean, mode and median"}
...
::: {.callout-definition title = "Variance and standard derivation"}
...
::: {.callout-definition title = "Quantiles"}
The **quantile** of order $\alpha$ of a random variable $Y$ is defined
as the value $y_\alpha$ such that
$$
...
Joint beliefs about $Y_1$ and $Y_2$ can be represented with probabilities. For example, for subsets $A\subset \Y_1$ and $B\subset \Y_2$, $\pr(\{Y_1\in A\} \cap \{Y_2 \in B\})$ represents our belief that $Y_1$ takes a value in $A$ and $Y_2$ takes a value in $B$. The $joint pdf$ or *joint density* of $Y_1$ and $Y_2$ is defined as 
...
::: {.callout-example title = "Education and Income Mobility"}
...
Joint distribution of education and income
Suppose a sociological study reports the following **joint distribution** of **parents’ education level** and **children’s income level** in a population as shown in the Table below
...
::: {.callout-definition title = "Joint cummulative distribution function and joint density function"}
...
+ a conditional density $p_{Y_2\mid Y_1}$ from $\pr(Y_2\le y_2\mid Y_1=y_1)\doteq F_{Y_2\mid Y_1}(y_2\mid y_1)$.
...
Then, in this case, we may threat $\theta$ as continuous random variable taking values in $\Theta = (0,1)$, and $Y$ as a discrete random variable taking values in $\Y = \{0,1,2,\dots,n\}$, where $n$ is the sample size. Bayesian estimation of the parameter $\theta$ derives from the calculate of $p(\theta\mid y)$ where $y$ is the obeserxd value of $Y$. In bayesian, this calculation first requires that we have a joint density $p(y,\theta$ representing our belief about $\theta$ and the survey outcome $Y$. Often, it is natural to cosnt5ruct this joint density from 
...
$$
\pr(Y_1 \in A_1,\dots,Y_n \in A_n \mid \theta) = \prod_{i=1}^n \pr(Y_i \in A_i \mid \theta).
$$
If we have independence property, then 
$$
\pr(Y_i\in A_i \mid \theta, Y_j\in A_j) = \pr(Y_i \in A_i \mid \theta),
$$
...
Let $Y_i\in\Y$ for all $i \in\{1,2,\dots,n\}$ be an exchangeable sequence of random variables. Then, there exists a parameter space $\Theta$ and a prior distribution $p(\theta)$ on $\Theta$ such that the joint distribution of $Y_1,\dots,Y_n$ can be represented as
$$
p(y_1,\dots,y_n) = \int_{\Theta} \left\{\prod_{i=1}^n p_Y(y_i \mid \theta)\right\} p(\theta) d\theta,
$$
where $p_Y(y \mid \theta)$ is a probability density function on $\Y$
for each $\theta \in \Theta$. The prior and sampling model depend on the form of the belief model $p(y_1,\dots,y_n)$.
:::
