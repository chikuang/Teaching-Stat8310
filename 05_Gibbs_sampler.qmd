...
This algorithm is called the **Gibbs sampler**. It generates a *dependent* sequence of parameter values
$$
\bigl\{ \phi^{(1)}, \phi^{(2)}, \ldots, \phi^{(S)} \bigr\}.
$$
:::

Below we give R code for implementing this Gibbs sampling scheme for the normal model with a semiconjugate prior distribution.
