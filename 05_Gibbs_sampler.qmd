...
The reason for writing the code this way is that $s^2$ and $\bar{y}$ do not change with each new $\theta$ value. Therefore computing
...
::: {.callout-algorithm title= "Gibbs sampling updates"}
...
::: {.callout-definition title="Markov property"}
...
$$
p(\boldsymbol{\phi}^{(s)} \mid \boldsymbol{\phi}^{(0)},\ldots,\boldsymbol{\phi}^{(s-1)})
=
p(\boldsymbol{\phi}^{(s)} \mid \boldsymbol{\phi}^{(
s-1)}).
$$
...
::: {.callout-definition title="Markov chain"}
...
This means that we can approximate the expectation $E[g(\boldsymbol{\phi})]$ using the sample average of the Gibbs samples
...
::: {.callout-example title="semiconjugate normal model"}
...
::: {.callout-note title="Key point of MC and MCMC"}
...
::: {.callout-note title="Bayesian analysis workflow"}
...
The purpose of MC or MCMC approximation is to obtain a sequence of parameter values
